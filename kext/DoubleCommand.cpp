...
bool com_baltaks_driver_DoubleCommand::start(IOService *provider)
{
	...
	IOLog("Starting DoubleCommand IOKit\n");
	if (!res) { return res; }
...
/*	for (int i = 0; i<MAX_KEYBOARDS; i++)
	{
	IOLog("keyboard %08X this is %08X", Keyboards[i].keyboard, this);
		if (Keyboards[i].keyboard == this)
		{
			Keyboards[i].event(target, eventType, flags, key, charCode, charSet, 
